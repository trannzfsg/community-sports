...
async function main() {
  ...
  console.log('registrations visible', regsSnap.size);
  if (!regsSnap.size) throw new Error('No registrations found to test.');

  const target = regsSnap.docs[0];
  const data = target.data();
  console.log('target registration', target.id, data);

  await updateDoc(doc(db, 'registrations', target.id), {
    organiserPaid: !data.organiserPaid,
    playerPaid: true,
  });
